...
def get_additive_mil_model_n_weights_1d(num_classes):
    model = DefaultMILGraph(
        featurizer =  ResNet1D(
                in_channels=13, 
                ...
                use_do=True),
        pointer=DefaultAttentionModule(hidden_activation = nn.LeakyReLU(0.2), hidden_dims=[256, 256], input_dims=256),
        classifier=AdditiveClassifier(hidden_dims=[256, 256], input_dims=256, output_dims=num_classes)
    )
    weights_path = "artifacts/nsclc/model_weights/wt_additive_mil.pth"
    return model, weights_path
