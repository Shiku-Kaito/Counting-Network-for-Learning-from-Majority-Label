...
class Attention(nn.Module):
    def __init__(self, num_class: int = 10, pool: str = "att"):
        super().__init__()
    ...
    def forward(self, x):
        ...
        y_inst = self.classifier(H)
        # y_inst = F.softmax(y_inst, dim=1)

        H = H.reshape(batch, num_ins, -1)

        A = self.attention(H)  # NxK
        A = torch.transpose(A, 2, 1)  # NxKxB
        A = F.softmax(A, dim=-1)  # softmax over B
        H = torch.matmul(A, H).squeeze(1)  # KxL

        y_bag = self.classifier(H)
        return {"bag": y_bag, "ins": y_inst}

        # else:
            # for idx, one_bag in enumerate(H):
            #     A = self.attention(one_bag)  # NxK
            #     A = torch.transpose(A, 1, 0)  # KxN
            #     A = F.softmax(A, dim=1)  # softmax over N

            #     M = torch.mm(A, H[idx])  # KxL
            #     if idx == 0:
            #         y_bag = self.classifier(M)
            #     else:
            #         y_bag = torch.cat((y_bag, self.classifier(M)), 0)
